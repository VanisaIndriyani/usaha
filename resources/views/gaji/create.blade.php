<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <div class="text-xl font-bold tracking-tight text-brand-navy dark:text-white">Tambah Gaji</div>
        </div>
    </x-slot>

    <div class="glass-card" x-data="gajiForm()">
        <div class="card-body">
            <form method="POST" action="{{ route('gaji.store') }}" class="space-y-5">
                @csrf
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="bulan" name="bulan" type="number" class="mt-1 block w-full" :value="old('bulan', $bulan)" min="1" max="12" required />
                            <x-input-error class="mt-2" :messages="$errors->get('bulan')" />
                        </div>
                        <div>
                            <x-input-label for="tahun" value="Tahun" />
                            <x-text-input id="tahun" name="tahun" type="number" class="mt-1 block w-full" :value="old('tahun', $tahun)" min="2000" max="2100" required />
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <select id="status" name="status" class="input mt-1">
                            <option value="belum_dibayar" @selected(old('status', 'belum_dibayar') === 'belum_dibayar')>belum_dibayar</option>
                    ...
                    <div>
                        ...
                        <x-text-input id="tanggal_bayar" name="tanggal_bayar" type="date" class="mt-1 block w-full" :value="old('tanggal_bayar')" />
                ...
                <div class="flex items-center gap-2">
                    ...
                    <a href="{{ route('gaji.index', ['bulan' => $bulan, 'tahun' => $tahun]) }}" class="btn-ghost">Batal</a>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        function gajiForm() {
            return {
                ...
                gajiHarian: {{ old('gaji_harian', 0) }},
                hariKerja: {{ old('hari_kerja', 0) }},
                bonus: {{ old('bonus', 0) }},
                updateGajiHarian() {
