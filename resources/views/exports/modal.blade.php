...
<head>
    ...
    <style>
        ...
        .summary td { padding: 3px 0; font-size: 10px; color: #475569; }
...
<body>
        ...
        <table class="brand">
            <tr>
                ...
                <td>
                    ...
                    <div class="h-sub">Laporan Modal</div>
                </td>
    ...
    <div class="kpi">
        ...
        @if ($byOwner->count() > 0)
            <table class="summary">
                @foreach ($byOwner as $name => $sum)
                    <tr>
                        <td>{{ $name }}</td>
                        <td class="right value">{{ $idr((int) $sum) }}</td>
                    </tr>
                @endforeach
            </table>
        @endif
    ...
    @if ($rows->count() === 0)
        <div class="empty">Belum ada data modal pada periode ini.</div>
    @else
        <table class="report">
            <thead>
                <tr>
    ...
    <script type="text/php">
        if (isset($pdf)) {
            $pdf->page_text(28, 820, "Usaha Baraya · Laporan Modal", null, 9, array(71, 85, 105));
