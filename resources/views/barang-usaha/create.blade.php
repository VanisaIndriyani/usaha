<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <div class="text-xl font-bold tracking-tight text-brand-navy dark:text-white">Tambah Barang</div>
        </div>
    </x-slot>

    <div class="glass-card">
        <div class="card-body">
            <form method="POST" action="{{ route('barang-usaha.store') }}" enctype="multipart/form-data" class="space-y-5">
                @csrf
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <x-text-input id="harga" name="harga" type="number" class="mt-1 block w-full" :value="old('harga')" min="0" required />
                        <x-input-error class="mt-2" :messages="$errors->get('harga')" />
                    </div>
                    <div>
                        <x-input-label for="jumlah" value="Jumlah" />
                        <x-text-input id="jumlah" name="jumlah" type="number" class="mt-1 block w-full" :value="old('jumlah', 1)" min="1" required />
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <x-text-input id="tanggal_beli" name="tanggal_beli" type="date" class="mt-1 block w-full" :value="old('tanggal_beli', now()->toDateString())" required />
                ...
                <div class="flex items-center gap-2">
                    ...
                    <a href="{{ route('barang-usaha.index') }}" class="btn-ghost">Batal</a>
                </div>
            </form>
        </div>
    </div>
